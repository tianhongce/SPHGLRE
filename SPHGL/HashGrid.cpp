...
bool HashGrid::operator=(HashGrid &hashGrid)
{
	...
	GridCell* next = NULL;
	GridCell* thisNext = NULL;
	for (int i = 0; i < hashGrid.hash.size(); i++)
	{
		//拷贝表首
		if (hashGrid.hash[i] != NULL)
		{
			this->hash[i] = new GridCell(hashGrid.hash[i]);
		...
		while (next != NULL)
		{
			thisNext->next = new GridCell(next);
			next = next->next;
	...
	for (int i = 0; i <= xSum; i++)
	{
		for (int j = 0; j <= ySum; j++)
		{
			for (int k = 0; k <= zSum; k++)
			{
				...
				//添加网格单元到哈希表
				gridCell = new GridCell(curx, cury, curz, idxGrid);
				this->AddHashCell(idxGrid, gridCell);
				curz += len;//当前z坐标
			}
			curz = oz;
			cury += len;//当前y坐标
...
GridCell* HashGrid::GetGridCell(GridIndex &idxGrid)
{
	//根据网格索引获取哈希索引
	int hashIndex = GetHashIndex(idxGrid);
	GridCell* gridCell = hash[hashIndex];
...
//	return true;
//}
bool HashGrid::AddHashCell(GridIndex idxGrid, GridCell* gridCell)
{
	if (gridCell == NULL)
		return false;
...
bool HashGrid::AddParticle(double x, double y, double z, int idx)
{
	...
	GridCell* gridCell = this->GetGridCell(idxGrid);
