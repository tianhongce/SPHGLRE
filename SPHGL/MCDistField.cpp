			...
			for (int k = 0; k < zSum; k++)
			{
				...
				*/
				...
				if (iv >= 8 && positive != 8 && negative != 8)
				{
					...
					{
						mesh.push_back(edgeVertexInter[triCell[itc]].x);
						mesh.push_back(edgeVertexInter[triCell[itc]].y);
						mesh.push_back(edgeVertexInter[triCell[itc]].z);
					}
					...
					for (int itcn = 0; itcn < 16 && triCell[itcn] != -1; itcn += 3)
					{
						...
						{
							meshVector.push_back(normal.x);
							meshVector.push_back(normal.y);
							meshVector.push_back(normal.z);
						}
					}
